...
impl Hub {
  ...
  async fn send_ignored(&self, ignore_client_id: Uuid, output: Output) {
    if self.output_sender.receiver_count() == 0 {
      return;
    }
  ...
  async fn process_post(&self, client_id: Uuid, input: PostInput) {
    ...
    // report post status
    self.send_targeted(client_id, Output::Posted(PostedOutput::new(message_output.clone())));

    // notify everyone about new message
    self.send_ignored(client_id, Output::UserPosted(UserPostedOutput::new(message_output)))
    .await;
  }
}